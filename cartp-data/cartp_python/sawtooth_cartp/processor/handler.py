...
class cartpTransactionHandler(TransactionHandler):
    ...
    def apply(self, transaction, context):
        ...
        # Send request to Ipfs_module with Private Key  
        if tnx_cmd == "crash":
            
            _sendDataPubeKeyAndSignature(options["signature"], options["dataPublicKey"])

def _sendDataPubeKeyAndSignature(sig, publicKey): # add DataPublicKey as paprameter

    HOST = 'module_ipfs' #'134.59.230.58'    # The remote host
    PORT = 81              # The same port as used by the server
    
    #LOGGER.debug(" ***************** SendInfo : {} {} ********************".format(HOST, PORT))

    #publicKey = 'Tonny Stark'
    inputData = {'PubKey' : publicKey, 'Signature' : sig}

    inputDataToString = str(inputData)
    bin_sig = bytes(inputDataToString, 'utf8')
    #bin_sig=json.dumps(inputData).encode('utf-8')

    #LOGGER.debug("=> => => bin_sig : {}".format(repr(bin_sig)))
    #LOGGER.debug("=> => => bin_sig : {}".format(bin_sig.decode("utf-8")))
    #bin_sig=json.dumps({'PubKey' : publicKey, 'Signature' : sig}).encode('utf-8')

    s = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
    s.connect((HOST, PORT))
    #bin_sig = bytes(sig, 'utf8')

    s.sendall(bin_sig)
    data = s.recv(1024)
    #LOGGER.debug("=> => => Data back : {}".format(repr(data)))
    #LOGGER.debug("=> => => Data data back: {}".format(data.decode("utf-8")))
    s.close()
